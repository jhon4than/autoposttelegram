    ...
    for(const source of sourcesToRun){
      ...
      try{
        ...
        for await(const message of client.iterMessages(source.peer_id,params)){
          ...
          const size=Number(message.file?.size||message.document?.size||0);if(size>SOURCE_MAX_FILE_MB*1024*1024){db.prepare('INSERT OR IGNORE INTO source_imports(source_id,message_id) VALUES(?,?)').run(source.id,message.id);sourceLog(source.id,'warning','oversize_skipped',`Vídeo de ${Math.round(size/1048576)} MB ignorado por ultrapassar o limite de ${SOURCE_MAX_FILE_MB} MB.`,message.id);continue;}
